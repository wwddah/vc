    ...
    if "all" in id:
      ...
      if str(query.from_user.id) in id.split('+')[1]:
         ...
         try:
           ...
           pass
      if query.from_user.id == 7861478788 or query.from_user.id ==7861478788:
         return await query.answer(q[:200], show_alert=True, cache_time=600)
  else:
    id = query.data.split("whisper")[0]
    if r.get(id):
      get = r.get(id)
      id = get.split('id=')[1].split('&')[0]
      if not 'all' in id and not str(query.from_user.id) in id and not query.from_user.id == 7861478788:
      ...
      await query.answer(
        ...
        results=[           
            InlineQueryResultArticle(
                ...
                thumb_url='https://k.top4top.io/p_2727oxo3z0.jpg',
                thumb_width=128, thumb_height=128,
                description='@marilinbot Hello @zvzzs',
                url='https://t.me/zvzzs',
                reply_markup=InlineKeyboardMarkup (
                [
